...
def _watch_and_upload(task_id: str, project_root: str, process=None, interval: int = 5, timeout: int = 600, idle_threshold: int = 60):
    """
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            ...
            if returncode is not None and returncode != 0:
                crud.update_task_status(db, task_uuid, "failed")
                print(f"❌ 任务 {task_id} 采集进程失败，退出码: {returncode}")
                push_event(task_id, f"ERROR:process exited with {returncode}")
            ...
            if new_files:
                ...
                for file_path in new_files:
                    ...
                    try:
                        ...
                        # SSE 推送：新截图入库
                        push_event(task_id, f"NEW_IMAGE:{total_new}")
            ...
            if time.time() - last_new_file_time > idle_threshold and process_done:
                ...
                if image_count == 0:
                    ...
                    push_event(task_id, "ERROR:no images collected")
                else:
                    crud.update_task_status(db, task_uuid, "completed")
        ...
        try:
            crud.update_task_status(db, task_uuid, "failed")
            push_event(task_id, "ERROR:collection timeout")
            push_event(task_id, "DONE")
        finally:
            db.close()


def start_collection_watcher(task_id: str, project_root: str, process=None):
    """
    启动后台线程监控截图目录
    """
    t = threading.Thread(
        target=_watch_and_upload,
        args=(task_id, project_root, process),
        daemon=True,
