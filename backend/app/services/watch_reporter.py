...
from app.services.task_planner import _planner_client
...
class WatchReporter:
    def summarize_daily(self, plan, run, primary_image, previous_summary=None) -> dict:
        ...
        fallback = self._fallback_daily(plan, run, design, ops, previous_summary)
        try:
...
不要输出Markdown，不要输出解释。"""
    ...
    def _fallback_daily(self, plan, run, design: str | None, ops: str | None, previous_summary=None) -> dict:
        ...
        return {
            ...
            "design_summary": design_text or "暂无可用设计分析。",
            "ops_summary": ops_text or "暂无可用运营分析。",
            "key_modules_json": [],
            "promotions_json": [],
